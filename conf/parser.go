...
func parsePrivateKeyBase64(s string) (*PrivateKey, error) {
	...
	if len(k) != PrivateKeyLength {
		return nil, &ParseError{l18n.Sprintf("Private key must decode to exactly 32 bytes"), s}
	}
	var key PrivateKey
	copy(key[:], k)
	return &key, nil
...
func parsePublicKeyBase64(s string) (*PublicKey, error) {
	...
	if len(k) != PublicKeyLength {
		return nil, &ParseError{l18n.Sprintf("Public key must decode to exactly 33 bytes"), s}
	}
	var key PublicKey
	copy(key[:], k)
	return &key, nil
...
func FromWgQuick(s string, name string) (*Config, error) {
	...
	for _, line := range lines {
		...
		if parserState == inInterfaceSection {
			switch key {
			case "privatekey":
				k, err := parsePrivateKeyBase64(val)
				if err != nil {
					return nil, err
				}
			...
			switch key {
			case "publickey":
				k, err := parsePublicKeyBase64(val)
				if err != nil {
					return nil, err
				}
				peer.PublicKey = *k
